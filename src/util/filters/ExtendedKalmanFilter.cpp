...
ExtendedKalmanFilter::ExtendedKalmanFilter() {
    ...
    covariance_estimate = Matrix(5, 5);
    process_covariance = Matrix(5, 5);
    observation_covariance = Matrix(5, 5);
...
void ExtendedKalmanFilter::update(ControlState u, VehicleState z, float delta_time) {
    ...
    innovation_state.set(1, 0, z.v_x);
    innovation_state.set(2, 0, z.y);
    innovation_state.set(3, 0, z.v_y);
    innovation_state.set(4, 0, z.theta);

    state = updateStateEstimate(kalman_gain, innovation_state);

    struct_state = {state.get(0, 0), state.get(2, 0), state.get(1, 0), state.get(3, 0),
                    state.get(4, 0)};
}
...
Matrix ExtendedKalmanFilter::getOptimalKalmanGain(Matrix residual_covariance) {
...
Matrix ExtendedKalmanFilter::updateStateEstimate(Matrix kalman_gain, Matrix y) {
 ...
 */
double ExtendedKalmanFilter::getGlobalTheta(double theta) { return 90.0 - theta; }
