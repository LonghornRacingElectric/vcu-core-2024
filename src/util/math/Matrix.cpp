...
int Matrix::columns() const { return (int)this->con[0].size(); }

int Matrix::rows() const { return (int)this->con.size(); }

void Matrix::set(int row, int col, float value) { con.at(row).at(col) = value; }

float Matrix::get(int row, int col) const { return con[row][col]; }
...
Matrix Matrix::divide(const Matrix &other) const {
    ...
    for (int row = 0; row < rows(); row++) {
        for (int col = 0; col < columns(); col++) {
            if (other.get(row, col) == 0 && get(row, col) == 0 || other.get(row, col) == 0) {
...
float Matrix::determinant() const { return 1.0f; }
...
Matrix Matrix::operator/(const Matrix &other) const { return divideDiagonal(other); }
